...
import { getAlerts, getCustomerTimeline, getTransactionById, patchAlert, postAlertAction } from '../services/api';
...
export default function InvestigationPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const matchingAlert = (await getAlerts())?.find((alert) => alert.transactionId === selected.id);
      setAlertId(matchingAlert?.id ?? null);

      const customerTimeline = await getCustomerTimeline(selected.customerId);
      const customerHistory = customerTimeline?.length ? customerTimeline.map((item) => ({
        date: item.timestamp,
        amount: item.amount,
  ...
  const handleAction = async (action) => {
    if (!alertId) return;
    const status = action === 'Allow' || action === 'Request Verification' || action === 'Simulated Restriction'
      ? 'Resolved'
      : 'Investigating';
    await patchAlert(alertId, status);
